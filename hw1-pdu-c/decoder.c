...
arp_packet_t *process_arp(raw_packet_t raw_packet) {
    ...
    arp_packet_t *arp_pkt = (arp_packet_t *)raw_packet;
    arp_pkt->eth_hdr.frame_type = ntohs(arp_pkt->eth_hdr.frame_type);
    arp_pkt->arp_hdr.htype = ntohs(arp_pkt->arp_hdr.htype);
    arp_pkt->arp_hdr.ptype = ntohs( arp_pkt->arp_hdr.ptype);
    arp_pkt->arp_hdr.op = ntohs(arp_pkt->arp_hdr.op);
 ...
 */
    ...
    char tadr[16];
    char tha[18];
    ip_toStr(ap.spa, sadr, sizeof(sadr));
    ip_toStr(ap.tpa, tadr, sizeof(tadr));

    mac_toStr(ap.sha, sha, sizeof(sha));
    mac_toStr(ap.tha, tha, sizeof(tha));
    printf("ARP DETAIL\n");
    printf("h_type: 0x%04x\n",ap.htype);
...
bool check_ip_for_icmp(ip_packet_t *ip){
    ...
    if(ip->ip_hdr.protocol == ICMP_PTYPE) return true;
...
icmp_packet_t *process_icmp(ip_packet_t *ip){
    ...
    //You do not need to allocate any memory. 
    icmp_packet_t *icmp_pkt = (icmp_packet_t *) ip;
    icmp_pkt->ip.eth_hdr.frame_type = ntohs(icmp_pkt->ip.eth_hdr.frame_type);
    icmp_pkt->ip.ip_hdr.identification = ntohs(icmp_pkt->ip.ip_hdr.identification);
    icmp_pkt->ip.ip_hdr.flags_and_fragment_offset = ntohs(icmp_pkt->ip.ip_hdr.flags_and_fragment_offset);
    icmp_pkt->ip.ip_hdr.header_checksum = ntohs(icmp_pkt->ip.ip_hdr.header_checksum);
    icmp_pkt->icmp_hdr.checksum = ntohs(icmp_pkt->icmp_hdr.checksum );
...
icmp_echo_packet_t *process_icmp_echo(icmp_packet_t *icmp){
    ...
    //You do not need to allocate any memory. 
    icmp_echo_packet_t *icmp_echo_pkt = (icmp_echo_packet_t *)icmp;

    icmp_echo_pkt->icmp_echo_hdr.id = ntohs(icmp_echo_pkt->icmp_echo_hdr.id);
    icmp_echo_pkt->icmp_echo_hdr.sequence = ntohs(icmp_echo_pkt->icmp_echo_hdr.sequence);
    icmp_echo_pkt->icmp_echo_hdr.timestamp = ntohl(icmp_echo_pkt->icmp_echo_hdr.timestamp);
    icmp_echo_pkt->icmp_echo_hdr.timestamp_ms = ntohl(icmp_echo_pkt->icmp_echo_hdr.timestamp_ms);
 ...
 */
    ...
    time_t seconds = (time_t)(icmp_packet->icmp_echo_hdr.timestamp); 
    struct tm timestamp_tm;
    gmtime_r(&seconds, &timestamp_tm);

    char timestamp_str[50];
    strftime(timestamp_str, sizeof(timestamp_str), "%Y-%m-%d %H:%M:%S", &timestamp_tm);
    sprintf(timestamp_str + strlen(timestamp_str), ".%05u", icmp_packet->icmp_echo_hdr.timestamp_ms);
